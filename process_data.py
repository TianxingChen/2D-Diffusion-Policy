...
def main():
    ...
    parser.add_argument('--task-name', type=str, default='block_hammer_beat',
                        help='The name of the task (e.g., block_hammer_beat)')
    parser.add_argument('--episode-number', type=int, default=50,
                        help='Number of episodes to process (e.g., 50)')
    args = parser.parse_args()

    task_name = args.task_name
    num = args.episode_number
    load_dir = f'../RoboTwin_private/data/{task_name}_pkl'
    
    total_count = 0

    save_dir = f'./data/{task_name}_{num}.zarr'
    ...
    head_camera_arrays, episode_ends_arrays, action_arrays, state_arrays, joint_action_arrays = [], [], [], [], []
    ...
    while os.path.isdir(load_dir+f'/episode{current_ep}') and current_ep < num:
        ...
        while os.path.exists(load_dir+f'/episode{current_ep}'+f'/{file_num}.pkl'):
            ...
            head_img = data['observations']['head_camera']['rgb']
            # head_img = cv2.resize(head_img, (128, 128))
            # head_img = einops.rearrange(head_img, 'h w c -> c h w')
            action = data['endpose']
    ...
    head_camera_chunk_size = (100, head_camera_arrays.shape[1], head_camera_arrays.shape[2])


    zarr_data.create_dataset('head_camera', data=head_camera_arrays, chunks=head_camera_chunk_size, dtype='float32', overwrite=True, compressor=compressor)
